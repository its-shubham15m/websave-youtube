...
if url:
    ...
    if yt:
        ...
        if download_option == "Video":
            # Try to find a stream with a better filename
            video_stream_with_filename = None
            for stream in yt.streams.filter(progressive=True, file_extension='mp4').order_by('resolution').desc():
                if stream.default_filename != "videoplayback.mp4":
                    video_stream_with_filename = stream
                    break

            if video_stream_with_filename:
                selected_stream = video_stream_with_filename
            else:
                # Use the first available stream as a fallback
                selected_stream = yt.streams.filter(progressive=True, file_extension='mp4').order_by('resolution').desc().first()
        ...
        if download_option == "Audio":
            ...
            if selected_audio_stream:
                ...
                # Set the download link with the video title as the filename
                audio_title = extract_video_title(url)
                if audio_title:
